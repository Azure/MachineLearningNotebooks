...
# ADDED BEGIN #
parser.add_argument('--enable_remote_debug', action='store_true',
                    help='Enables VScode to attach remotely to the process. Port will be indicated at stderr or stdout.')
#  ADDED END  #
...
print("Remote debug : " + str(REMOTE_DEBUG))
if REMOTE_DEBUG:
    print("Remote debug enabled.")
    localMPIRank = hvd.local_rank()
    globalMPIRank = hvd.rank()
    import ptvsd
    debug_port_number = 5678 + globalMPIRank
    ptvsd.enable_attach(address=("0.0.0.0", debug_port_number))
    print("GR {0} : waiting for debugger to attach to process on port {1}...".format(globalMPIRank, debug_port_number))
    ptvsd.wait_for_attach()
    if ptvsd.is_attached():
        print("GR {0} : Debugger attached to process on port {1}!".format(globalMPIRank, debug_port_number))
    else:
        print("GR {0} : Debugger not attached to port {1} - moving forward...".format(globalMPIRank, debug_port_number))
# ADDED END #
